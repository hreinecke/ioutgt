...
| Flag | Default | Effect |
|------|---------|--------|
| `--backend` | `memory` | `memory` / `null` / `file` (regular file or bdev, O_DIRECT) / `sheepdog:HOST[:PORT][/VDI[@TAG]][?nolock]` (one cluster VDI, or one namespace per VDI if none is named; each taken under the shared VDI lock unless `?nolock`) |
