...
| Flag | Default | Effect |
|------|---------|--------|
| `--backend` | `memory` | `memory` / `null` / `file` (regular file or bdev, O_DIRECT) / `sheepdog:HOST[:PORT][/VDI[@TAG][%ACL]][?nolock]` (one cluster VDI, or — if none is named — one subsystem per cluster ACL object holding one namespace per VDI in it; each taken under the cluster's VDI lock unless `?nolock`) |
