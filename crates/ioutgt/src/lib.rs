...
/// Start a target's control thread; returns the bound address (for
/// ephemeral-port tests). The queue-thread pool is spawned lazily on the
/// first connection and reclaimed after an idle grace period. Runs until
/// the process exits.
